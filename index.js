...
io.on("connection", (socket)=> {
  ...
  socket.on('start-game', (_room) => {
    ...
    for(var p of players){
      if(p.room === _room)
      {
        ...
        p.addCards(cards)
        pir.push({id:p.id, name:p.name, cards:p.cards.length, turn:p.turn, kadi:p.kadi})
  ...
  socket.on('kadi-state', (_room) => {
    let message = ""
    let pir = []
    for (let i = 0; i < players.length; i++){
      if(players[i].id === socket.id && players[i].kadi === false){
        console.log(`${socket.nickname} is kadi!!!`)
        message = `${socket.nickname} is kadi!!!`
        players[i].setKadi(true)
        pir.push({id:players[i].id, name:players[i].name, cards:players[i].cards.length, turn:players[i].turn, kadi:true})
      }
      else if(players[i].id === socket.id && players[i].kadi === true){
        console.log(`False kadi alarm ${socket.nickname} was just bluffing!!!`)
        message = `False kadi alarm ${socket.nickname} was just bluffing!!!`
        players[i].setKadi(false)
        pir.push({id:players[i].id, name:players[i].name, cards:players[i].cards.length, turn:players[i].turn, kadi:false})
      }else{
        pir.push({id:players[i].id, name:players[i].name, cards:players[i].cards.length, turn:players[i].turn, kadi:false})
      }
    }
  ...
  socket.on('play-cards', (_room, _cards, reqSuit) => {
    ...
    if(player.command === "play"){
      ...
      io.to(player.id).emit('wrong-move', "It is not your turn")
    ...
    if(response.error){
      ...
      io.to(player.id).emit('wrong-move', message)
      return
    }else{
      //Get the next player
      ...
      if(!gameOver){
        ...
        io.in(_room).emit('cards-played', {tc,pir,cards})
      }
    }
  })
    ...
    if(player.turn){
      for(let i = 0; i < players.length; i++){
        if(players[i].id === socket.id && players[i].command === "play"){
          ...
          players[i].command = "play"
          io.to(players[i].id).emit('my-turn')
          message = `It's still ${socket.nickname} turn`
        }
        if(players[i].room === _room){
          pir.push({id:players[i].id, name:players[i].name, cards:players[i].cards.length, turn:players[i].turn, kadi:players[i].kadi})
        }
